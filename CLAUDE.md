...
Requires the Anytype desktop app on the host; from the devcontainer the API is `http://host.docker.internal:31009` (compose sets `ANYTYPE_API_BASE_URL` and `ANYTYPE_API_KEY_FILE=.devcontainer/secrets/anytype_api_key` already — `localhost` does *not* work from inside the container). The suite reuses one space named exactly `GC-E2E` and **resets it before and after each run** (the local API cannot delete spaces) — spike artifacts (S9/S10 sets, chats, todos) do not survive a run; the spike scripts reseed themselves. It is intentionally slow: the live server throttles writes to ~1 req/s. After the WP14 sidecar cutover (ADR 019: the `anytype` compose service now starts with every up/rebuild; cutover = bot account + key swap + base-URL flip) the endpoint becomes `http://anytype:31012` — a headless bot-account node with the rate limit disabled — and the desktop app is no longer needed for E2E.
...
- **The consumer of every tool response and error string is an LLM.** Tool docstrings are prompts (`interface/server.py`); errors derive from `GraphContextError` and echo allowed values so the model can self-correct. Response-budget shaping lives in `interface/presenters.py`, not in tested logic. (The old per-response `[project | focus | recent]` header was removed 2026-07-06 as token waste; the session still tracks focus/recent for query defaults and future features.)
