...
async def world(writer: NodeWriter) -> World:
    mira = await writer.create_node(
        NodeDraft(NodeType.CHARACTER, name="Mira", summary="Exiled siege engineer.")
    )
    undercroft = await writer.create_node(
        NodeDraft(NodeType.LOCATION, name="The Undercroft", summary="Vaults beneath Brakk."),
    ...
    fall = await writer.create_node(
        ...
        links=[LinkSpec(EdgeType.PARTICIPATED_IN, other=mira.id, outgoing=False)],
    )
    ashbrand = await writer.create_node(
        NodeDraft(NodeType.ITEM, name="Ashbrand", summary="A blade quenched in ash."),
        links=[LinkSpec(EdgeType.POSSESSES, other=mira.id, outgoing=False)],
    )
