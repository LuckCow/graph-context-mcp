...
The inspection server ([ADR 025](docs/adr/025-inspection-server.md)) runs automatically inside `serve` (the devcontainer publishes it to the host at `http://127.0.0.1:8765/`), or standalone via `python -m graph_context.orchestrator.inspect_server`. It is dependency-free and serves three things: an **eval dashboard** at `/` (every case with its latest verdict and history, per-run grade/judge/prompt detail, one-click trial transcripts — `GC_EVAL_ROOT` points it at the artifacts, default `evals`), the **live turn-log viewer** at `/logs`, grouping the diary into one collapsible card per user request and live-tailing via SSE (filter by session/mode, search, errors-only), and the **prose review page** at `/prose` ([ADR 050](docs/adr/050-section-review-and-prose-page.md), mobile-friendly): per-paragraph blame coloring, revision timelines with word-level diffs, and per-section status/intent marks for the tracked node types of every live space (`serve` only — the standalone server has no live spaces and shows an empty state). Reads need no auth; setting a mark is the server's only write and needs `GC_PROSE_TOKEN` set (unset = the page is read-only) plus a same-origin request — the page asks for the token once and remembers it. Point the server elsewhere with `--log` / `--port` / `--eval-root` or `GC_LOG_VIEWER_HOST` / `GC_LOG_VIEWER_PORT`. No server needed for the diary either: open `src/graph_context/orchestrator/turn_log_viewer.html` directly in a browser and pick a `turns.jsonl` file.
...
| `orchestrator/inspect_server.py` | Inspection server (eval dashboard + turn-log viewer + prose review) | Stdlib-only; SSE live tail with shrink→reset; hosts `inspect.html` + `turn_log_viewer.html` + `prose.html`; `POST /api/prose/mark` is the one write (origin check + `GC_PROSE_TOKEN`, ADR 050); `turn_log_server.py` is its back-compat shim |
| `orchestrator/prose_bridge.py` | Prose page ↔ bot-loop seam (WP43) | Registry of per-space handles; every page read/write crosses via `run_coroutine_threadsafe` onto the owning loop, marks under the route lock |
