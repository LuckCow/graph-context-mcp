...
* Call `services.persister.note_mutation()` after writes once the
  persister is wired in server.py lifespan (marked inline below).
* `record_prose` and `only_stale` are WP3 surface already present here;
  `get_node include_prose` still needs NodeReader work (see node_reader).
"""
...
async def get_node_tool(
    ...
    view = await services.reader.get_node(
        node_id, edge_type_filter=_edge_type_set(edge_types)
    )
